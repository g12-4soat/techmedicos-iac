resource "aws_eks_cluster" "eks-techmedicos" {
...
resource "aws_eks_node_group" "techmedicos-node" {
  ...
  depends_on = [aws_eks_cluster.eks-techmedicos]
}
...
resource "aws_eks_addon" "aws_ebs_csi_driver" {
  ...
  depends_on = [aws_eks_node_group.techmedicos-node]
}

resource "aws_security_group" "eks-sg" {
  name        = "eks-default-sg"
  description = "Default security group for the EKS cluster"
  vpc_id      = aws_eks_cluster.eks-techmedicos.vpc_config[0].vpc_id
}
